...
int main() {
        ...
        for(int i=0; i<size_x; i++) {
            for(int j=1; j<size_y-1; j++) {
                ...
                //right wall
                    ...
                    TNew[i][j] = ((T[i][j+1] + T[i][j-1])/2 + T[i-1][j] + bi*Tinf) / (2 + bi);
                }
            }
        }
        //       printf("error: %f\n", error);
    ...
    FILE *fp1 = fopen("resultCoordinatesConvection.txt", "w");
    FILE *fp2 = fopen("resultMatrixConvection.txt", "w");
